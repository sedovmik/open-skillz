                ...
                <div class="text-sm font-medium text-gray-900 relative group">
                  {#if game.status === 'completed' && game.score}
                    ...
                    {#if game.ratingUpdate}
                      <div class="z-10 w-64 p-2 mt-2 bg-gray-800 text-white text-xs rounded shadow-lg">
                        {#each Object.entries(game.ratingUpdate) as [playerId, [before, after]]}
                          <div class="mb-1">
                            {getPlayerNames([parseInt(playerId)])}: {before.mu.toFixed(2)} → {after.mu.toFixed(2)}
                          </div>
                        {/each}
                      </div>
                    {/if}
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700">
                {getPlayerNames(selectedGameId ? $gameStore.find(g => g.id === selectedGameId)?.teamA || [0] : [0])} Score
              </label>
              <input
                type="number"
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700">
                {getPlayerNames(selectedGameId ? $gameStore.find(g => g.id === selectedGameId)?.teamB || 0 : 0)} Score
              </label>
              <input
                type="number"
